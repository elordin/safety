        ...
        public override void Update()
        {
            ...
            _stateMachine.
                Transition(
                    ...
                    to: State.OnEP,
                    guard:
                        MeasuredDistance <= Model.Z + MeasuredSpeed * MeasuredSpeed / 2 / Model.A + 2 * Model.C * MeasuredSpeed + MeasuredSpeed * (Model.T + Model.C)).
                Transition(
                    from: State.OnEP,
                    to: State.EPtoAP,
                ...
                Transition(
                    from: State.EPtoAP, 
                    to: State.OnAP,
                    guard: MeasuredDistance <= Model.Z + MeasuredSpeed * MeasuredSpeed / 2 / Model.A + 2 * Model.C * MeasuredSpeed).
                Transition(
                    from: State.OnAP, 
                    to: State.APtoBEP,
                ...
                Transition(
                    from: State.APtoBEP,
                    to: State.OnBEP,
                    guard: MeasuredDistance <= Model.Z + MeasuredSpeed * MeasuredSpeed / 2 / Model.A).
                Transition(
                    from: State.OnBEP, 
                    to: State.BEPtoGP,
